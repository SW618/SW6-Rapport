...
\subsubsection*{Pair Programming}\label{subsubsec:pairprogramming}
Aside from using some of the methods of Scrum, we also use a technique from ExtremeProgramming, called Pair Programming.
Pair programming means having two programmers work on the same programming tasks on the same computer, without anything else.
The two programmers can switch between who is doing the actual typing, but meanwhile it makes the two programmers discuss what they are doing and why.
The result of this is that the code will usually be that the code is of better quality than if the tasks were completed by a single programmer.
When the programmers are not skilled in pair programming it might reduce the throughput of the code, but as a higher quality of code has many benefits, e.g. less bugs that will need to be fixed later, it is definitly a valid method of programming.
The method was first described by Don Wells when he presented Extreme programming in 1999, and more information about pair programming can be found on his website \cite{pairprogramming}.
...
\input{mainmatter/introduction/toolsindex.tex}