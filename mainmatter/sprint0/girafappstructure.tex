...
However it is recommended that all the applications is download before any usage. \todo{Kilde nødvendig?}
\begin{description}
	\item[Giraf (Main Launcher)] The Giraf Launcher replaces the default launcher of the device and is used to launch the applications of the Giraf Application Suite as-well as 3rd party applications. 
	Additionally the options menu and administration panel is opened from here. 
	\item[Administration Tool] The Administration Tool is primarily used by guardians to configure the applications each citizen can utilize. 
	...
	\item[Category Game] The Category Game (also known as ``Kategorispillet'' in danish) is a game which ask the player to group up different pictograms into categories either predefined or defined in the Category Manager. 
	...
	\item[Life Story] Life Story (also known as ``Livshistorier'' in danish) is similar to the Sequence application but is used to remember what actions was taken by a citizen throughout the day. 
	\item[Voice Game] Voice Game (also known as ``Stemme Spillet'' in danish) is a game in which the purpose is to control a car, either avoiding obstacles or gathering stars, only using your voice. 
\end{description}}
