...
For this sprint allocate 60 EP are available, the estimate is made with time spend on courses in mind, the 60 EP include not only solving our own tasks but also reviewing tasks from other groups and writing the report.
...
The following description is the sprint backlog for sprint 1 and contains priority, point estimation, user story and an expanded problem explanation for each task.
...
\begin{description}[style=unboxed]
    ...
    \item[{[}HIGH{]} Picto Search - Responsive Search] \hfill \\
        ...
        This task is estimated at 8 half work days, the task includes changing what triggers the search method as well as changing the GUI to be more intuitive.
    \item[{[}HIGH{]} General - Use consistent file encoding] \hfill \\
        ...
        This task is estimated at 1 half work day, doing this for every repository is bound to give conflicts with other groups working in overlapping repository, as such this task is derived to writing a guide such that others can change the encoding of a file when they access a file that is not using UTF-8 encoding.
...
\end{description}
The combined point value of the tasks for sprint 1 sum to 49 points leaving 11 points for review and report.
