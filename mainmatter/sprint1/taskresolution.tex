...
\end{itemize}
As can be inferred by the version numbers updating oasisLib is likely to provide cause errors as it is two non backwards-compatible versions behind.
localDB and meta-database both pertain to the database where meta-database defines all the tables and keys and localDB pertain to symmetricDS. 
oasisLib contains models and controllers for said models, updating this particular library causes a varierty of errors in the repositories sequence and sequence-viewer.\todo[inline]{Der eksistere ingen dokumentation overhovedet om hvad de 3 depencies er, detter er blot hvad jeg har kunne inferere gennem de methods som er i filerne, what do boys? - M}
\begin{description}
