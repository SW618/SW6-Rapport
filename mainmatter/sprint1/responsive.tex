...
Instead of the big slow search spinner, which blackened out and locked the screen and can be seen on \myref{fig:screenshot_searchspinner}, a new smaller text message is instead displayed which tells the user that the application is searching and what is is searching for. 
...
This has been removed and it is now only visible when there is actually text which can be cleared from the search field, as can be seen on \myref{fig:screenshot_newstartup} and it it still there on \myref{fig:screenshot_newsearch}.
