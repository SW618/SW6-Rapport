...
\kim{Having read this section then there are one important element that I am missing, you argue that the real problem is that the user is not getting the right kind of feedback and that is what curses the uses to feel that ``noting is happening''. In order to fully believe that argument then I would like to see some numbers on how long a search takes. Just make some small test to see if the average search time is actually below 4 seconds. If not then I think you need to argue that there are two problems, and in this sprint you only address the ``wrong feedback'' problem.}
...
Therefore the amount of time spent waiting for the application should be reduced, such that there is no need for using other methods of feedback when the search finally returns\kim{Just below you argue the exact opposite and decide that there should be feedback. This is very confusing for the read}.
...
This means that there is near instantaneous feedback for the user, which might give the feeling that the application is reacting to whatever the user is doing, and it might no longer feel to users like nothing is happening. \kim{It seems an obvious question to ask if you tested this, or do you plan to test this?}

The search button can still be used, and will bring up the old search spinner as usually, but the button has been moved from the middle of the display to the right side of the display as can be seen on \myref{fig:screenshot_newstartup}. \kim{Why does the search button not just evoke the responsive search functionality? If the problem was that the old search feedback felt slow and unresponsive! }
This is a better fit as it resembles other common search engines such as Google Search, and it is also the recommended way to display a search-field according to the Niels Norman Group\footnote{https://www.nngroup.com/articles/magnifying-glass-icon/}. \kim{Here you might also refer to the gestalt principles in particular the similarity principle. }
...
\kim{you ``missspelled'' the following words: recognisable, actoion, pricinple, qucikly. Just FYI.}
...
\kim{I love the clear analysis and design of this section. It is well grounded in theory. }
\kim{No implementation details? If you do not think that it is of interest then you should not add any implementation details.}