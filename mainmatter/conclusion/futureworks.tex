...
\kim{You should not only point out the problems but also suggest solution, where it make sense. for each problem that you discuss then I would like an analysis of what features/qualities a solution should have and a proposal of a design that conforms to these requirements.}
...
\end{enumberate*} \kim{Why the numbers? Just list the improvements.}
...
\kim{You often explain the problems based of your work, this is good, however I would like to see more references to the sections where you developed the current solutions.}
...
This however is not enough as different data exists for other applications in GIRAF, namely maps and game settings for both Voicegame and Categorygame, as well as categories themselves. \kim{I am surpised that you mention voice game since it is not part of the bundle any more, I assume that it have its own storage solution now and it should not be synced with the remaining GIRAF apps.}
...
The conflict handling also still need to fine tuned such that less conflicted copies will be created. \kim{Last time I read the conflict handling section then I found your solution quite simple and lacking. I would like to read about some furture work for this.}
...
\kim{What would a new and better solution for this be? Maybe present how the endpoint for such a feature would look like, along with argument for why that would be a good idea.}
...
\kim{I now understand the problem, how would a solution to this problem look like.}
...
\subsection*{Launcher}
While our involvement on the Launcher was primarily in regards to offline mode, most offline mode changes that must occur the following semester should be implementing the offline policy\kim{Please add a reference to this policy.} we established in each app when the REST API is used.
