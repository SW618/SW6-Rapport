...
The original motivation for making the REST API were enabling security and synchronization.
We have previously documented how the previous solution was unable to achieve either.
Up until now we had only considered authentication security.
...
To reconcile we decided to hold this meeting such that we could design a proper solution which would follow all the requirements.
...
Access to another user's private information is in direct conflict with the security model proposed earlier by SW615F16.
...
We decide to invent three access levels: One for all public information, one for all department shared information and one for private pr. user information.
We call these:
\begin{itemize}
    \item Public
    \item Protected
...
Our figure shows that it’s possible to synchronize some data in the background, even before the user logs in.
Since a large number of pictograms, provided by Giraf, are stored as public we expect this to help with synchronization speed on first login.
...
\begin{figure}[!ht]
    ...
    \end{tikzpicture}
    \caption{The imagined login workflow with the REST API.}
