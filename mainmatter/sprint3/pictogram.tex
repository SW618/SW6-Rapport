...
\begin{center}
   \userstory{As a developer I would like an endpoint for Pictograms, such that I can retrieve them from GIRAF.}
\end{center} 

\kim{I dont understand the problem, all pictograms are downloaded locally before logon. Why would you want to search online?}
...
Therefore a goal of the REST API for pictograms is to provide a search method, such that PictoSearch will return the same results. \kim{Same results for what? If it always resunts the same results then it is a sucky service ;)}
The endpoint will also be used in the case that a user opens e.g. a week schedule which contains a pictogram that the device have not stored locally, the endpoint will then retrieve this single pictogram rather than returning a list of pictograms. \kim{I think you should make these scenarios even clearer.}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/pictogrammodel.pdf}
...
Significant fields which require further explination, will be elaborated in the following descriptions           
...
\kim{To improve this paragraph then you could discuss why you use Lazy instead of Eager}
...
\kim{This is a good section, however it can be a bit hard for the reader to understand why you specially made the rest methods that you did. You could fix that by, somewhere, describe the scenarios that you need to support. e.g. During logon, when a device need to check if it have all pictograms downloaded.}
...
We create a merge method which simply takes the input retrieved from JSON and swaps\kim{When you say swap do you mean overwrite? If not then I do see how swapping anything merges the data.} the information, such that the new information is set to the data objects in the database.
