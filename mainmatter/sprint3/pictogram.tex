...
\end{lstlisting}
\todo[inline]{Skal vi forklare hvad factory pattern er mere abstrakt? I så fald hvor dybdegående skal det være?}
...
\end{tabular}
\end{table}\todo{Sass gør tabellen lækker ... please...LÆKKERFY DEN!!!!}
...
\myref{lst:getallPictograms} shows the method which handles a GET request on the path ``/'' which is seen from the annotations \texttt{@GET} and \texttt{@Path("/")}.
