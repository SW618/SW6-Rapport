...
\subsubsection{Hibernate Annotations/Often used Hibernate Annotations}
Throughout the different layers a varierty of annotations are used, these are shortly explained here.
\todo[inline]{Hvor relevant dette afsnit er ved jeg ikke, det kan også være det skal ligges sammen med hibernate istedet, hvis dette afsnit skrives skal et par sætninger i resten af afsnittes ændres så de blot refere til annotation forklaringer når en annotation er nævnt frem for at forsøge at forklare den.}
...
\begin{description}
	...
	If a class has a many-to-many relationship with another class and this relationship-table needs to contain additional information, e.g. an index for the relationship, a table that includes the index needs to be created as well.
	As we are developing incrementally on the REST API any alterations to a class may require subsequent alterations to the related table, this should also be done in this step.
...
\begin{description}
	...
	Instead the way we test the service layer is to manually test whether the methods return the expected JSON format or not, e.g. using a web-browser like Google Chrome.
\end{description}
\noindent
...
\subsection{The new Model for REST API}\todo{add something bout users}
...
\todo{indsæt klassediagram her} A week schedule consists of 7 week days one for each day of the week which creates a relation from a \texttt{weekSchedule} to several \texttt{weekDay}s.
...
\begin{itemize}
	...
	\item \texttt{Choice}s
\end{itemize}
In considering this we created a super class which all these can inherit from, and from this we can make it so a week schedule for each day has a list of this super class, which we call \texttt{Frame}.
...
Therefore \texttt{Sequence}s and \texttt{Pictogram}s inherit from yet another superclass called \texttt{PictoFrame}, that inherits from \texttt{Frame}, which encapsulate the common data between \texttt{Sequence}s and \texttt{Pictrogram}s.
...
The \texttt{weekday} has a relationship-class: \texttt{weekdayFrame} to bind these \texttt{Frame}s together with a weekday which helps to preserve the order of frames on a weekday as well as being able to save the progress of a weekschedule.
...
With the model described the folling sections goes into more detail about the specific endpoints.