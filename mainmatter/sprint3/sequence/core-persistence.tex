...
In \myref{pictogramendpoint} we showed how to model af \texttt{One-to-Many}, and a \texttt{One-to-One}, this section will who how we model a \texttt{Many-to-Many} while making sure the order of the list is intact.
...
A \texttt{@ManyToMany} annotation is used here aswell, but we also specify a \texttt{@JoinTable} annotation and a \texttt{@OrderColumn}, the latter will be explained in the next section \textit{Ordering the Frames}.
The \texttt{@JoinTable} annotation is much like the \texttt{@JoinColumn} as explained in \myref{pictogramendpoint}, but instead a table is created, which is given a name \texttt{Sequence\_\_Frame}, and also two join columns \texttt{sequence\_id} and \texttt{frame\_id}.
These \texttt{JoinColumns} work as explained in \myref{pictogramendpoint}, but there is also an \texttt{inverseJoinColumn} which is simply the column for the entity being joined with \texttt{Sequence} which is \texttt{Frame}.
...
As explained in \myref{subsec:seqcore} it is important that the order of the elements are correct, and we can do this in two ways:
...
The alternative solution is quite simple, and can be seen on \myref{lst:sequnce-frame} and \myref{lst:sql-frame-sequence}.
