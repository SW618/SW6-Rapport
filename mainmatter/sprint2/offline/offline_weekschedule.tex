...
\kim{I think this is a weird place to have this descussion. For me then a policy is something that applies for the entire application/ecosystem (GIRAF), each app then need to implement this policy, but the policy should apply to all the apps. In the introduction section of this user story then the policy problem was the first problem mentioned, however it does not placed as the first subsection. I think this is confusing and should be aligned. }
